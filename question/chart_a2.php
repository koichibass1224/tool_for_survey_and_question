...
//1.  DB接続します
try {
  $pdo = new PDO('mysql:dbname=question_db;charset=utf8;host=localhost','root','root');
} catch (PDOException $e) {
  exit('DBConnectError:'.$e->getMessage());
}

//２．データ取得SQL作成
$stmt1 = $pdo->prepare("SELECT a2 FROM question_table;");
//$stmt1 = $pdo->prepare("SELECT theme FROM question_table;");
...
var group = arr.reduce(function (result, current) {
  var element = result.find(function (p) {
    return p.a2 === current.a2//最新データで上書き。重複しない。
  });
  if (element) {
    element.count ++;
  } else {
    result.push({
      count: 1,
      a2: current.a2,//自分で決める。そのまま配列になる。
    });
  }
  return result;
}, []);
...
array.unshift(group[key].a2)//push（末尾に追加）でもok
};
console.log(array);
