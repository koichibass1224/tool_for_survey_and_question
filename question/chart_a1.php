...
//２．データ取得SQL作成
$stmt1 = $pdo->prepare("SELECT a1 FROM question_table;");
...
var group = arr.reduce(function (result, current) {
  var element = result.find(function (p) {
    return p.a1 === current.a1//最新データで上書き。重複しない。
  });
  if (element) {
    element.count ++;
  } else {
    result.push({
      count: 1,
      a1: current.a1,//自分で決める。そのまま配列になる。
    });
  }
  return result;
}, []);
...
array.unshift(group[key].a1)//push（末尾に追加）でもok
};
console.log(array);
